...
const CORE_BENEFITS = [
  {
    id: 'AI_PLAN',
    title: 'Brain dump your startup chaos → Get a plan in 90 seconds',
    description:
      'Drop in launches, bugs, growth tasks, and random ideas. Resurgo turns them into milestones, weekly targets, and today’s top actions instantly — no setup rabbit hole.',
    outcome: 'First execution plan in under 10 minutes',
    icon: 'goals' as PixelIconName,
    terminalCmd: '> plan --goal "launch startup" → 12 milestones generated',
  },
  {
    id: 'AI_COACHING',
    title: 'AI coaching for solo execution',
    description:
      'When you are context-switched and overloaded, Resurgo pulls you back to one highest-leverage move. Coaching adapts to your actual shipping patterns, not motivational quotes.',
    outcome: 'Clear next move every day, even on low-energy days',
...
const ADHD_FEATURES = [
  ...
  { title: 'Flexible Scheduling', desc: 'Bad day? Missed a habit? That\'s okay. Resurgo adapts your plan without judgment. Consistency over perfection.', icon: 'timer' as PixelIconName },
  ...
  { title: 'Brain Dump to Clarity', desc: 'Pour everything out of your head. Resurgo organizes the chaos into a prioritized, time-blocked plan.', icon: 'robot' as PixelIconName },
];
...
const ACCESS_TIERS = [
  ...
  {
    tier: 'LIFETIME',
    price: { monthly: `$${FOUNDING_LIFETIME_PRICE_USD}`, annual: `$${FOUNDING_LIFETIME_PRICE_USD}` },
    period: { monthly: 'one-time', annual: 'one-time' },
    earlyAccess: true,
    originalPrice: `$${FOUNDING_LIFETIME_REGULAR_PRICE_USD}`,
    ...
    specs: [
      ...
      `Founding price: $${FOUNDING_LIFETIME_PRICE_USD} (${FOUNDING_LIFETIME_COPY.toLowerCase()})`,
    ],
    cta: 'LOCK_LIFETIME_ACCESS',
    highlight: true,
  },
];
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 selection:bg-orange-600/40 selection:text-white">
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            offers: [
              ...
              { '@type': 'Offer', price: '9.99', priceCurrency: 'USD', name: 'Pro Monthly', description: 'All 5 AI coaches, unlimited history, advanced analytics, priority support' },
              { '@type': 'Offer', price: String(FOUNDING_LIFETIME_PRICE_USD), priceCurrency: 'USD', name: 'Founding Lifetime', description: `Everything in Pro, forever. One-time payment. ${FOUNDING_LIFETIME_COPY}. Regular price $${FOUNDING_LIFETIME_REGULAR_PRICE_USD}.` },
            ],
      ...
      <main>
              ...
              <div className="max-w-[640px]">
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                  ...
                  <TermLinkButton href="/sign-up?plan=lifetime" variant="gold" size="lg">
                    Lifetime Access — $89 (founding price)
                  </TermLinkButton>
                </div>
        ...
        <div className="overflow-hidden border-y border-zinc-800/50 bg-zinc-950/80 py-3">
            ...
            {[...Array(2)].map((_, copy) => (
              <div key={copy} className="flex shrink-0 items-center gap-8 px-4">
                {[
                  ...
                  <span key={`${copy}-${item}`} className="font-pixel text-[0.35rem] tracking-[0.25em] text-zinc-600">
                    {item}
                  </span>
                ))}
              </div>
        ...
        <section id="coaches" className="bg-black px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="mx-auto max-w-6xl">
            <div className="mb-10 text-center">
              ...
              <h2 className="mt-2 font-pixel text-lg text-zinc-100 sm:text-xl">
                5 AI coaches. One connected execution system.
              </h2>
              ...
              {AI_COACHES.map((coach) => (
                <article
                  ...
                  className={cn(
                    ...
                    coach.accent === 'pink' && 'border-pink-500 hover:bg-pink-950/10',
                  )}
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className={cn(
                      ...
                      coach.accent === 'pink' && 'text-pink-400',
                    )}>{coach.name}</span>
                    <span
                      className={cn(
                        'shrink-0 border-2 px-2 py-0.5 font-pixel text-[0.35rem] tracking-widest',
                        ('comingSoon' in coach && coach.comingSoon)
                          ? 'animate-pulse border-cyan-900 bg-cyan-950/40 text-cyan-400'
                          : coach.free
                            ? 'border-green-900 bg-green-950/40 text-green-500'
                            : 'border-orange-900 bg-orange-950/40 text-orange-500',
                      )}
                    >
                      {('comingSoon' in coach && coach.comingSoon) ? 'SOON' : coach.free ? 'FREE' : 'PRO'}
                    </span>
                  </div>
                  ...
                  {plan.earlyAccess ? (
                    <div className="mt-3">
                      ...
                      <p className="mt-1 font-terminal text-sm text-zinc-400">
                        ...
                        <span className="ml-2 text-yellow-500">save 55%</span>
                      </p>
                      <p className="mt-1 font-pixel text-[0.5rem] tracking-widest text-zinc-400">
                        Regular price after the first {plan.spotsLeft?.toLocaleString()} relaunch signups
                      </p>
                    </div>
                  ) : plan.highlight ? (
                    <div className="mt-3">
            ...
            <div className="mt-8 flex justify-center">
              <div className="inline-flex items-center gap-4 border-2 border-yellow-800 bg-yellow-950/20 px-6 py-3">
                ...
                <div className="border-l border-yellow-800 pl-4">
                  <span className="font-pixel text-sm text-yellow-300">{FOUNDING_LIFETIME_LIMIT}</span>
                  <span className="ml-1 font-pixel text-[0.45rem] tracking-widest text-zinc-400">founding relaunch spots only</span>
                </div>
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="mx-auto max-w-6xl">
            <div className="mb-10 text-center">
              ...
              <p className="mx-auto mt-4 max-w-xl font-terminal text-lg leading-relaxed text-zinc-200">
        ...
        {/* ────────────────── BUILT IN PUBLIC ────────────────── */}
        <section className="border-y border-zinc-900 bg-zinc-950/40 px-4 py-12 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            <p className="font-pixel text-[0.5rem] tracking-widest text-orange-500">BUILD_LOG</p>
            <h2 className="mt-3 font-pixel text-lg text-zinc-100 sm:text-xl">
              Built in public. Shipping every day.
            </h2>
            <p className="mx-auto mt-4 max-w-2xl font-terminal text-lg leading-relaxed text-zinc-400">
              Resurgo isn&apos;t built by a faceless corporation. It&apos;s built by a solo founder who uses the product every single day.
              Every feature ships because a real user needed it — and that user is often the person writing the code.
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              {[
                { label: 'OPEN_ROADMAP', value: '✓' },
                { label: 'DAILY_UPDATES', value: '✓' },
                { label: 'USER_FEEDBACK_DRIVEN', value: '✓' },
                { label: 'INDIE_BUILT', value: '✓' },
              ].map((badge) => (
                <span key={badge.label} className="border border-zinc-700 bg-zinc-900/50 px-4 py-2 font-pixel text-[0.38rem] tracking-widest text-zinc-400">
                  <span className="text-green-500 mr-1">{badge.value}</span> {badge.label}
                </span>
              ))}
            </div>
          </div>
